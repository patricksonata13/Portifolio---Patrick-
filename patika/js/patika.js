...
/* ============================================ */
/* FUNCIONALIDADES EXCLUSIVAS DA ABA ROTEIRO    */
/* ============================================ */

// Só executa se estiver na aba roteiro
if (document.getElementById('roteiro-tab-content')) {
    
    function detectarTipoLinha(texto) {
        texto = texto.trim();
        if (/^(INT\.|EXT\.)/i.test(texto)) return "cena";
        if (texto.length > 0 && texto === texto.toUpperCase() && texto.length < 30) return "personagem";
        return "acao";
    }
    
    function aplicarFormatacao(linha, tipo) {
        linha.classList.remove("patika-cena", "patika-personagem", "patika-dialogo", "patika-acao");
        switch(tipo) {
        ...
        editor.addEventListener('keydown', function(e) {
            if (e.key === 'Enter') {
                setTimeout(() => {
                    ...
                    const tipo = detectarTipoLinha(linha.innerText);
                    aplicarFormatacao(linha, tipo);
                    ...
                    if (tipo === 'personagem') {
                        setTimeout(() => {
                            ...
                            const sel = window.getSelection();
                            range.selectNodeContents(dialogo);
                            range.collapse(true);
                            sel.removeAllRanges();
                            sel.addRange(range);
                        }, 10);
...
window.salvarVersao = function() {
    const editor = document.getElementById('roteiro-editor');
    if (editor) {
        localStorage.setItem('roteiro_backup', editor.innerHTML);
        alert('✅ Versão salva!');
    }
};

window.exportarRoteiro = function() {
    const editor = document.getElementById('roteiro-editor');
    if (editor) {
...
window.toggleModoFoco = function() {
    const editor = document.getElementById('roteiro-editor');
    if (editor) {
        editor.style.background = editor.style.background === '#f8f9fa' ? 'white' : '#f8f9fa';
    }
};
...
window.toggleModoEscuro = function() {
    const editor = document.getElementById('roteiro-editor');
    if (editor) {
        editor.style.background = editor.style.background === '#1a1a1a' ? 'white' : '#1a1a1a';
        editor.style.color = editor.style.color === '#e0e0e0' ? '#2f2f2f' : '#e0e0e0';
    }
};

/* ============================================ */
/* FUNCIONALIDADES FINAL DRAFT - ABA ROTEIRO    */
/* ============================================ */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function aplicarFormatacao(linha, tipo) {
        ...
        switch(tipo) {
            ...
            case "dialogo":
                ...
                break;
            case "transicao":
                linha.classList.add("patika-transicao");
                break;
            default:
    ...
    function atualizarListaCenas() {
        ...
        listaCenas.innerHTML = '<h3 style="margin-top:0;">📋 CENAS</h3>';
        ...
        for (let i = 0; i < linhas.length; i++) {
            if (linhas[i].classList.contains('patika-cena')) {
                ...
                item.style.borderLeft = '3px solid #00A86B';
                ...
                item.onclick = () => {
                    ...
                    setTimeout(() => linhas[i].style.backgroundColor = '', 500);
                };
    ...
    function atualizarEstatisticas() {
        ...
        const palavras = texto.trim().split(/\s+/).filter(Boolean).length;
        const paginas = Math.ceil(palavras / 250);
        const tempo = Math.ceil(palavras / 150);
        ...
        const statusBar = document.querySelector('div[style*="justify-content: space-between"][style*="margin-top: 10px"]');
        if (statusBar) {
            statusBar.innerHTML = `
                <span>Linhas: ${linhas.length}</span>
                <span>Palavras: ${palavras}</span>
                <span>Páginas: ${paginas}</span>
                <span>Tempo: ${tempo} min</span>
            `;
        }

        // Atualizar estatísticas
        const statCenas = document.getElementById('stat-cenas');
        if (statCenas) statCenas.innerText = cenas;
        
        const statPersonagens = document.getElementById('stat-personagens');
        if (statPersonagens) statPersonagens.innerText = personagens;
        
        const statDialogos = document.getElementById('stat-dialogos');
        if (statDialogos) statDialogos.innerText = dialogos;
        
        const statFalas = document.getElementById('stat-falas');
        if (statFalas) statFalas.innerText = dialogos;
    }
...
window.salvarVersao = function() {
    ...
    const versoes = JSON.parse(localStorage.getItem('patika_versoes') || '[]');
    versoes.unshift({
...
window.toggleModoEscuro = function() {
    const editor = document.getElementById('roteiro-editor');
    if (!editor) return;
    if (editor.style.background === '#1a1a1a') {
...
window.inserirElemento = function(tipo) {
    const editor = document.getElementById('roteiro-editor');
    if (!editor) return;
    ...
    switch(tipo) {
        ...
        case 'dialogo':
            ...
            novaLinha.innerText = 'Digite o diálogo...';
            break;
        case 'acao':
    ...
    const range = document.createRange();
    const sel = window.getSelection();
    range.selectNodeContents(novaLinha);
    range.collapse(false);
    sel.removeAllRanges();
    sel.addRange(range);
    novaLinha.focus();
...
style.textContent = `
    ...
    .patika-acao { margin-left: 0; }
    .patika-transicao { text-transform: uppercase; color: #666; margin-top: 12px; }
`;
